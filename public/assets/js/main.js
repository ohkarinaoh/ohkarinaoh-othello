...
/* Set up the socket.io connection to the servr */
let socket = io();
...
socket.on('player_disconnected', (payload) => {
    ...
    let newHTML = '<p class = \'left_room_response\'>'+payload.username+' left the chat room.(There are '+payload.count+' users in the this room)</p>';
    let newNode = $(newHTML);
    newNode.hide();
    $('#messages').prepend(newNode);
    newNode.show("fade",500);
...
socket.on('game_update', (payload) => {
    ...
    if (payload.game.whose_turn === 'white'){
        $("#my_color").append('<h4> It is Sun\'s turn</h4>');
    }
    else if (payload.game.whose_turn === 'black'){
       $("#my_color").append('<h4 id="my_color">I am Moon\'s turn</h4>');
    }
    else {
        $("#my_color").append('<h4>Error: Don\'t know whose tun it is</h4>');
        }
