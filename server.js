...
/* Set up a registery of player information and their socket id*/
let players = [];
...
*/
            ...
            /* Socket did join room */
                ...
                for (const member of sockets) {
                    ...
                    /* Tell everyone that a new user has joined the chat room */
                     ...
                     serverLog('join_room succeed', JSON.stringify(response));
                     if (room !=="Lobby"){
    ...
    socket.on('invite', (payload) => {
        ...
        if ((typeof requested_user == 'undefined') || (room === null) || (requested_user === "")){
            response = {
                result: 'fail',
                message: 'client did not request a vaild user to invite to play'
            }
            socket.emit('invite_response', response);
            serverLog('invite command failed', JSON.stringify(response));
            return;
    ...
    socket.on('uninvite', (payload) => {
        ...
        if ((typeof requested_user == 'undefined') || (room === null) || (requested_user === "")){
            response = {
                result: 'fail',
                message: 'client did not request a vaild user to uninvite to play'
            }
            socket.emit('uninvited', response);
            serverLog('uninvite command failed', JSON.stringify(response));
            return;
...
*/
    ...
    socket.on('send_chat_message', (payload) => {
        ...
        if ((typeof username == 'undefined') || (username === null)) {
            response = {};
            response.result = 'fail';
            response.message = 'client did not send a valid username as a messageto as source';
            socket.emit('send_chat_message_response', response);
            serverLog('send_chat_message command failed', JSON.stringify(response));
            return;
        }
    ...
    socket.on('play_token', (payload) => {
        ...
        if ((typeof username == 'undefined') || (username === null)) {
            response = {};
            response.result = 'fail';
            response.message = 'play_token commnad did not send come from a registered username';
            socket.emit('play_token_response', response);
            serverLog('play_token command failed', JSON.stringify(response));
            return;
        }
        ...
        if ((typeof row == 'undefined') || (row === null)) {
            response = {};
            response.result = 'fail';
            response.message = 'There was no valid row game associated with the play_token command';
            socket.emit('play_token_response', response);
            serverLog('play_token command failed', JSON.stringify(response));
            return;
        }
        ...
        if ((typeof column == 'undefined') || (column === null)) {
            response = {};
            response.result = 'fail';
            response.message = 'There was no valid column game associated with the play_token command';
            socket.emit('play_token_response', response);
            serverLog('play_token command failed', JSON.stringify(response));
            return;
        }
        ...
        if ((typeof color == 'undefined') || (color === null)) {
            response = {};
            response.result = 'fail';
            response.message = 'There was no valid color game associated with the play_token command';
            socket.emit('play_token_response', response);
            serverLog('play_token command failed', JSON.stringify(response));
            return;
        }
        ...
        if (color === 'white') {
            ...
            flip_token('w',row,column,game.board);
            game.whose_turn = 'black';
            game.legal_moves = calculate_legal_moves('b',game.board);
        }
        else if (color === 'black') {
            game.board[row][column] = 'b';
            flip_token('b',row,column,game.board);
...
/* Return ture if r + dr supports playing at r and c + dc supports playing */
function adjacent_support(who,dr,dc,r,c,board){
    ...
    /* Check to make sure that the there is space for a mathcing color to capture tokens*/
...
function send_game_update(socket, game_id, message) {
    ...
    io.of('/').to(game_id).allSockets().then((sockets)=> {
        ...
        console.log("game update " + game_id);
    })
